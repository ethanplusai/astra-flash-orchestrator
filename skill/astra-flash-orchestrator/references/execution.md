...
The default is one native child using the installed worker model in the current workspace for one coherent
...
Parallelism is opt-in per plan. Use at most two worker writers by default, and only

...
## Astra-led planning and installed worker implementation
...
bundles to the native `astra_flash_builder` agent using the backend recorded in the installed routing binding.
Use one worker by default; do not create an agent for each tiny coding step.
...
Do not switch the root to the worker model, silently fall back to a different worker model,
...
uses the selected installed backend. Router mode requires its reviewed DeepSeek
route; native mode requires the built-in OpenAI provider in the active parent
session. Check routing.json and references/routing.md before delegation. Obey
the user's data-sharing and spending restrictions. Installation is not evidence of a
